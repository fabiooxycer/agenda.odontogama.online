<?php
  ...
  $conn = mysqli_connect($sistema->server, $sistema->user, $sistema->pass, $sistema->bd) or die(mysqli_error($conn));
	...
	if($_GET['confirm_del'] == 'delete') {
        mysqli_query($conn, "DELETE FROM laboratorios_procedimentos_status WHERE codigo_procedimento = ".$_GET['codigo_procedimento']);
        mysqli_query($conn, "DELETE FROm laboratorios_procedimentos WHERE codigo = ".$_GET['codigo_procedimento']);
	}
	$r = '';
	if(isset($_POST['Salvar'])) {
        if($_POST['procedimento'] != '') {
            mysqli_query($conn, "INSERT INTO laboratorios_procedimentos (codigo_paciente, codigo_dentista, codigo_laboratorio, procedimento, datahora) VALUES (".$_GET['codigo'].", ".$_SESSION['codigo'].", ".$_POST['codigo_laboratorio'].", '".utf8_decode ( htmlspecialchars( utf8_encode($_POST['procedimento']) , ENT_QUOTES | ENT_COMPAT, 'utf-8') )."', NOW())");
            $strScrp = 'Ajax("pacientes/laboratorio", "conteudo", "codigo='.$_GET['codigo'].'&acao=editar&codigo_procedimento='.mysqli_insert_id().'")';
        } else {
            $r = '<font color="#FF0000">';
        }
	}
...
<?php
    $query = mysqli_query($conn, "SELECT * FROM laboratorios ORDER BY nomefantasia");
...
<?php
    $query = mysqli_query($conn, "SELECT tlp.*, td.nome dentista, tl.nomefantasia FROM laboratorios_procedimentos tlp INNER JOIN dentistas td ON tlp.codigo_dentista = td.codigo INNER JOIN laboratorios tl ON tlp.codigo_laboratorio = tl.codigo ORDER BY datahora DESC");
    echo mysqli_error();
    while($row = mysqli_fetch_assoc($query)) {
