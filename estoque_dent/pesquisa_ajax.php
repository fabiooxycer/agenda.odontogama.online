...
?>
    <tr>
      ...
      <td><input style="width:88px;" type="text" class="form-control" size="13" name="quantidade" id="quantidade" value="<?php echo $conta->RetornaDados('quantidade')?>" onblur="Ajax('estoque_dent/atualiza', 'conta_atualiza', 'codigo=<?php echo $conta->RetornaDados('codigo')?>&estoque='+this.value)" <?php echo ((!verifica_nivel('estoque', 'E'))?'disabled':'')?>></td>
      <td><?php echo ((verifica_nivel('estoque', 'A'))?'<a href="javascript:Ajax(\'estoque_dent/extrato\', \'conteudo\', \'codigo='.$conta->RetornaDados('codigo').'" onclick="return confirmLink(this)"><button class="btn btn-danger"><span class="glyphicon glyphicon-trash"></span></button></a>':'')?></td>
    </tr>
<?php
	}
	if($odev == $impar) {
...
?>
      ...
      <td width="140" align="right"><a href="relatorios/estoque_dent.php?sql=<?php echo $sql?>" target="_blank"><button class="btn btn-warning"><span class="glyphicon glyphicon-print"></span> <?php echo $LANG['stock']['print_report']?></button></a></td>
    </tr>
  </table>
