  ...
  $(function(){
    ...
    $("a#baixar").click(function(){
      ...
      swal({
        ...
        if(dismiss === true)
        {
          Ajax('contaspagar/atualiza', 'conta_atualiza', 'codigo='+codigo+'&datapagamento='+$("#dataPagamento").val()+'&formaPagamento='+$("#formaPagamento").val());
...
<table class="table table-hover">
  ...
  if(isset($_GET[procurar]))
  {
    ...
    if($campo != "") $complemento.="AND fornecedor='$campo' ";
  ...
  $sql = "SELECT contaspagar.* FROM contaspagar $complemento ORDER BY datavencimento ASC, descricao DESC";
  ...
  for($i = 0; $i < count($lista); $i++) {
   ...
   $carrFornecedor = $fornecedor->LoadFornecedores($conta->RetornaDados("fornecedor")); 
   ...
   <tr>
    ...
    <td width="25%" align="left"><?php echo utf8_decode($conta->RetornaDados('descricao'));?></td>
    <td width="2%" align="left"><?php echo $LANG['general']['currency'].' '.money_form($conta->RetornaDados('valor'))?></td>
    <td width="25%" align="left"><?php echo $fornecedor->RetornaDados('nomefantasia'); ?></td>
