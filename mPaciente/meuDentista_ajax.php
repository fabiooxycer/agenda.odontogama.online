		...
		if ($_POST[sosenha] == 'true') {
			...
			if($j == 0) {
                ...
                for ( $i = 0 ; $i <= 6 ; $i++ ) {
                    ...
                    mysqli_query ( $sql );
                    echo mysqli_error() ? $sql . '; ' . mysqli_error() . '<br/> ' : '';
...
<script type="text/javascript">
  ...
  $(function(){
    ...
    $("#cep").mask("00.000-000");
    $("#telefone1, #celular, #telefone2").mask("(00) 0 0000-0000");
  ...
  <table class="table">
    <tr>
      <td style="border-top:0;">
        ...
        <span class="style1"><?php echo $LANG['professionals']['personal_information']?></span>
      </td>
    </tr>
  </table>
        ...
        <table class="table">
          ...
          <tr>
            <td><?php echo $r[1]?>* <?php echo $LANG['professionals']['name']?><br />
                <label>
                  <input required name="nom" value="<?php echo $row[nome]?>" <?php echo $disable?> type="text" class="form-control" id="nom" size="50" maxlength="80" />
                </label>
                <br />
            </td>
...
?>
          <td><?php echo $LANG['professionals']['zip']?><br />
            ...
            <td><?php echo $LANG['professionals']['address1']?><br />
              <input required name="endereco" value="<?php echo $row[endereco]?>" <?php echo $disable?> type="text" class="form-control" id="endereco" size="50" maxlength="150" /></td>
            </tr>
            <tr>
            <td><?php echo $LANG['professionals']['address2']?><br />
              <input required name="bairro" value="<?php echo $row[bairro]?>" <?php echo $disable?> type="text" class="form-control" id="bairro" /></td>
          
            <td><?php echo $LANG['professionals']['city']?><br />
                <input required name="cidade" value="<?php echo $row[cidade]?>" <?php echo $disable?> type="text" class="form-control" id="cidade" size="30" maxlength="50" />
              <br /></td>
            <td><?php echo $LANG['professionals']['state']?><br />
                <input required name="estado" value="<?php echo $row[estado]?>" <?php echo $disable?> type="text" class="form-control" id="estado" maxlength="50" />
            </td>
          
            <td><?php echo $LANG['professionals']['country']?><br />
                <input required name="pais" value="<?php echo $row[pais]?>" <?php echo $disable?> type="text" class="form-control" id="pais" size="30" maxlength="50" />
            </td>
          ...
          <tr>
            ...
            <td><?php echo $LANG['professionals']['phone2']?><br />
              <input name="telefone2" value="<?php echo $row[telefone2]?>" <?php echo $disable?> type="text" class="form-control" maxlength="13" onKeypress="return Ajusta_Telefone(this, event);" /></td>
            </tr><tr>
...
?>       
            ...
            <td><?php echo $LANG['professionals']['parents_name']?><br />
                <label>
                  <input required name="nomemae" value="<?php echo $row[nomemae]?>" <?php echo $disable?> type="text" class="form-control" id="nomemae" size="50" maxlength="80" />
                </label>
                <br />
                <label></label></td>
...
<?php
    if ( isset ( $_GET['codigo'] ) ) {
        for ( $i = 0 ; $i <= 6 ; $i++ ) {
            $exists = mysqli_num_rows ( mysqli_query ( "SELECT * FROM dentista_atendimento WHERE codigo_dentista = " . $_GET['codigo'] . " AND dia_semana = " . $i ) );
            if ($exists <= 0) {
                mysqli_query ( "INSERT INTO dentista_atendimento ( codigo_dentista , dia_semana , hora_inicio , hora_fim , ativo ) VALUES
    ...
    for ( $i = 0 ; $i <= 6 ; $i++ ) {
        $weekday[$i] = mysqli_fetch_assoc ( mysqli_query ( "SELECT * FROM dentista_atendimento WHERE codigo_dentista = " . $_GET['codigo'] . " AND dia_semana = " . $i ) );
    }
...
?>
              <tr style="margin-top:3px;">
                  ...
                  for ( $i = 0 ; $i <= 6 ; $i++ ) {
                  ?>
                      <td align="center"><input disabled type="checkbox" name="weekday[<?php echo $i?>]" value="1" <?php echo (!isset($_GET['codigo']) || $weekday[$i]['ativo'] == 1 ) ? 'checked="checked"' : ''?>/></td>
                  <?php
                  }
                  ?>
              </tr>
              <tr>
                  ...
                  ?>
                      <td align="center">
                          <select disabled name="hora_inicio[<?php echo $i?>]" class="form-control" style="margin-top:3px;">
                              <?php
                              for ( $j = 7 ; $j <= 22 ; $j++ ) {
                                  for ( $k = 0 ; $k < 60 ; $k += 15 ) {
                          ...
                          <select disabled name="hora_fim[<?php echo $i?>]" class="form-control" style="margin-top:3px;">
                              <?php
                              for ( $j = 7 ; $j <= 22 ; $j++ ) {
                                  for ( $k = 0 ; $k < 60 ; $k += 15 ) {
                                      $hora = str_pad ( $j , 2 , '0' , STR_PAD_LEFT ) . ':' . str_pad ( $k , 2 , '0' , STR_PAD_LEFT ) . ':00';
