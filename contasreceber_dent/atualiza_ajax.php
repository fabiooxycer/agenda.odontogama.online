<?php
	...
	if($infor["ordem"] != "") $os->calcularComissao($infor["ordem"], "entrada", $infor["valor"]);

	$verificarUltima = mysqli_fetch_row(mysqli_query($conn, "SELECT * FROM parcelas_ordem WHERE id_ordem='$infor[ordem]' and status='0'"));

	if($verificarUltima[0] == "") mysqli_query($conn, "UPDATE tb_ordens SET status='1' WHERE id='$infor[ordem]'");

	mysqli_query($conn, "UPDATE contasreceber SET status='1' WHERE codigo='$codigo'")or die(mysqli_error($conn)); // remove do contas a receber.
	mysqli_query($conn, "UPDATE parcelas_ordem SET status='1' WHERE id_ordem='$infor[ordem]' AND data='$infor[datavencimento]'");
	mysqli_query($conn, "INSERT INTO caixa (data, dc, valor, descricao) VALUES ('$infor[datapagamento]', '+', '$infor[valor]', '$infor[descricao]')")or die(mysqli_error($conn)); // adiciona no fluxo de caixa
